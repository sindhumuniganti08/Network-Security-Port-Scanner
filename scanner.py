...
# Standard Known Port Services Mapping
PORT_SERVICES = {
...
class NetworkPortScanner:
    ...
    def resolve_target(self) -> Tuple[bool, str]:
        """Resolves target hostname to IPv4 address with error handling."""
        try:
    ...
    def grab_banner(self, ip: str, port: int) -> str:
        """Attempts to grab service banner from open port."""
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.settimeout(1.5)
                s.connect((ip, port))
                # Probe HTTP vs General Banner
    ...
    def scan_single_port(self, port: int) -> Dict[str, Any]:
        """Scans a single TCP port and returns port state details."""
        result = {
            "port": port,
            "state": "closed",
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                elif res == 11 or res == 10060:  # Timeout / filtered
    ...
    def scan_ports(self, port_list: List[int], progress_callback=None) -> Dict[str, Any]:
        """Runs concurrent port scan across specified list of ports."""
    ...
    def parse_port_range(range_str: str) -> List[int]:
        """Parses port specifications like '80,443,8000-8080' into integer list."""
        ports = set()
